...
import nz.dataview.websyncclient.clientinterface.Service;
 ...
 * @version 1.1.3
 */
public class Client extends UnicastRemoteObject implements Service {

   /**
    * The WebSYNC version.
    */
   public static final String WEBSYNC_VERSION = "1.1.3";
   /**
    * The logger!
    */
   ...
   public Client() throws RemoteException {
      try {
         ...
         systemConfigFilePath = System.getProperty("nz.dataview.websyncclient.sysconf_file", "C:/Program Files/WebSYNCClient/config/system.properties");
         initialiseConfig();
         initialiseLogger();
         initialiseProxy();
      } catch (IOException e) {
...
//	    initialiseProxy();
   ...
   /**
    * Attempts to restart the JVM on which this program runs, by using the 
    * WrapperManager static method <code>restart()</code>.
    * 
    * @throws java.rmi.UnmarshalException with cause of java.io.EOFException indicates success
    * @throws java.rmi.RemoteException	  if something went wrong
    */
   public void restart() throws UnmarshalException, RemoteException {
      if (logger.isTraceEnabled()) {
         logger.trace("Entered Client.restart()");
      }
      logger.info("Waiting for uploader and downloader threads to finish");
      finishThreads();
      ...
      if (logger.isTraceEnabled()) {
         logger.trace("Exiting Client.restart()");
      }
   }
...
//      }      
   ...
   public void startRMIService() throws Exception {
      if (logger.isTraceEnabled()) {
         logger.trace("Entered Client.startRMIService()");      // Create and install a security manager
      }
      if (System.getSecurityManager() == null) {
         System.setSecurityManager(new RMISecurityManager());
         if (logger.isDebugEnabled()) {
            logger.debug("System.getSecurityManager() returned null, created and set new RMISecurityManager");
         }
      }
      ...
      try {
         ...
         if (logger.isDebugEnabled()) {
            logger.debug("RMI Registry created successfully");         // Bind this object instance to the name "WebSYNCClientService"
         }
         Naming.rebind("WebSYNCClientService", this);
         if (logger.isDebugEnabled()) {
            logger.debug("Service bound in registry successfully");
         }
      } catch (Exception e) {
         logger.error("RMI error: " + e.getMessage() + e.getStackTrace() + ", my state: " + this);
         throw e;
      }

      if (logger.isTraceEnabled()) {
         logger.trace("Exiting Client.startRMIService()");
      }
   }
   ...
   private void verifyConfig() throws Exception {
      ...
      String sysConfFile = System.getProperty("nz.dataview.websyncclient.sysconf_file");
      File file = null;
      if (sysConfFile != null) {
   ...
   public String getProxyPw() {
      ...
      String ret = Utilities.simpleDecrypt(appConfig.getProperty("nz.dataview.websyncclient.proxy_pw"));
      if (ret == null) {
         ret = "";
      }
      if (logger.isTraceEnabled()) {
   ...
   public String getAuthenticationKey() {
      ...
      String ret = Utilities.simpleDecrypt(appConfig.getProperty("nz.dataview.websyncclient.authentication_key"));
      if (ret == null) {
         ret = "";
      }
      if (logger.isTraceEnabled()) {
   ...
   public String getSmtpPassword() {
      ...
      String ret = Utilities.simpleDecrypt(appConfig.getProperty("nz.dataview.websyncclient.smtp_password"));
      if (ret == null) {
         ret = "";
      }
      if (logger.isTraceEnabled()) {
