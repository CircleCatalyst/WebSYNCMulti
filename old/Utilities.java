...
public class Utilities {
   ...
   /**
    * Determines whether the provided filename should be uploaded or not.
    * Currently checks the list of files that were uploaded successfully, but were
    * not deleted from the uploaddir for whatever reason.
    * 
    * @param   filename the file to check
    * @param   list	the list of files not to be uploaded (provide null if none exists)
    * @return		true if this file should NOT be uploaded, false otherwise	
    */
   public static boolean dontUploadFile(String filename, String[] list) {
      if (logger.isTraceEnabled())
      {
          String msg="Entered Utilities.dontUploadFile() with String: ";
          if(filename !=null)
          {
              msg+=filename;
          } else
	 ...
	 for (int i = 0; i < list.length; i++) {
	    if (list[i].equals(filename)) {
	       ...
	       if (logger.isDebugEnabled())
		  logger.debug("File " + filename + " matches entry in list of files to not upload (" + list[i] + ")");
      ...
      if (logger.isTraceEnabled())
	 logger.trace("Exiting Utilities.dontUploadFile() with boolean: " + ret);
